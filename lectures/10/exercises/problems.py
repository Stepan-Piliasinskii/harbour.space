...
def threaded_square_map(values: list[int]) -> list[int]:
    ...
    for i, val in enumerate(values):
        thread = threading.Thread(target=square, args=(i, val))
        threads.append(thread)
        thread.start()
...
async def async_tag_fetch(labels: list[str], delay: float = 0.01) -> list[str]:
    ...
    async def fetch_and_tag(label: str) -> str:
        await async_simulated_long_fetch(label)
        return f"done:{label}"

    return await asyncio.gather(*(fetch_and_tag(label) for label in labels))
...
async def async_blocking_double(values: list[int]) -> list[int]:
    ...
    async def fetch_and_double(val: int) -> int:
        res = await asyncio.to_thread(simulated_long_fetch, val)
        return res * 2

    return await asyncio.gather(*(fetch_and_double(val) for val in values))
